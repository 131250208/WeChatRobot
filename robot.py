...
class Robot(Job):
    ...
    自定义消息处理函数
    '''
    def toLiujishou(self, msg: WxMsg, img_id, img_extra):
        """
        处理留几手消息
        """
        # 下载图片
        sv_dir = "/".join(img_extra.split("/")[:-1])
        img_sv_path = self.wcf.download_image(img_id, img_extra, sv_dir)
        
        # 打分
        storyteller = ImgStoryTeller() # v_model="OpenGVLab/InternVL2-26B"
        self.LOG.info(f"留几手看图说话-图片路径：{img_sv_path}")
        story_gen = storyteller.generate_story_with_rating(img_sv_path, stream=True)
        print("思考中：")
        ...
        if msg.from_group():
            ...
            if msg.is_at(self.wxid):  # 被@
                if msg.type == 49:
                    ...
                    self.LOG.info(f"id: {msg.id}\ntype: {msg.type}\ncontent: {msg.content}\nroomid: {msg.roomid}\nfrom_group: {msg.from_group()}\nfrom_self: {msg.from_self()}\nfrom_user: {msg.sender}\nfrom_nickname: {self.allContacts[msg.sender]}")  # 打印信息
                    print("--------------------------------")
                    print(f"id: {msg.id}\ntype: {msg.type}\ncontent: {msg.content}\nroomid: {msg.roomid}\nfrom_group: {msg.from_group()}\nfrom_self: {msg.from_self()}\nfrom_user: {msg.sender}\nfrom_nickname: {self.allContacts[msg.sender]}")  # 打印信息
                    print("--------------------------------")
                    self.toAt(msg)
                ...
                if msg.type == 3:
                    print(f"图片：{msg}")
                    reply_gen = self.toLiujishou(msg, msg.id, msg.extra)
                    reply_txt = ""
                    for chunk in reply_gen:
                        reply_txt += chunk
                    self.sendTextMsg(reply_txt.strip(), msg.roomid, msg.sender)
                else:
                    self.toChengyu(msg)
    ...
    def sendTextMsg(self, msg: str, receiver: str, at_list: str = "") -> None:
        ...
        if ats == "":
            ...
            self.LOG.info(f"To {receiver}: {ats} {msg}")
